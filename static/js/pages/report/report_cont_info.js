$(function (param) {
    let pageView = (function (param) {
        ...
        let itemsPerPage=10;
        let pageView = {};
        // let Arr=["ADVANCE_RENT", "ADVANCE_ELECTRICITY", "ADVANCE_SERVICE"];
        let heaArr=["contNo","companyName","brandName","itemName","contTypeName","storeNos","rentTypeName","points",
            "contBeginDate","contEndDate","rentBegin","decorateStage","statusName","storeType","incomeName",
            "taxType","turnover","lowestTurnover","structureSquare","shareSquare","rentSquareType","practicalSquare","createdDate"];
        let tableTitle = ["合同号","商户名称","品牌","商场名称","合同类型","铺位号","扣点方式","扣率",
            "合同开始时间","合同结束时间", "起租日期","装修期","合同状态","商铺类型","收入类型","计税类型",
            "最低月销售额","最低营业额连续时间(月)","建筑面积","实用面积","计租面积类型","计租面积","登记时间"];
        ...
        pageView.dropDown = function () {
            ...
            $('#largeNumBtn').next('ul').on('click',function (event) {
                ...
                if(target.is('a')){
                    ...
                    pageb = 1;
                    $('.zl-page-num-input').val(1);
                    console.log(pageView.reportData());
                    Api.again(pageView.reportData());
                }
            })
        }
        ...
        pageView.init = function (param) {
            Api.locaShow();
            Api.init('/report_web/contInfoReport/getReportData.htm',pageView.reportData(),'reportData',tableTitle);
    ...
    $(document).ready(function (param) {
        ...
        $('#exportAll').click(function () {
            ...
            formPost("/report_web/contInfoReport/exportAll.htm", pageView.reportData());
        });
        ...
        $('#export').click(function () {
            ...
            formPost("/report_web/contInfoReport/export.htm", pageView.reportData());
        });
