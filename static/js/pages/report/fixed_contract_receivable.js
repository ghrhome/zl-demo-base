$(function (param) {
    let pageView = (function (param) {
        ...
        let itemsPerPage=10;
        let pageView = {};
        let dataView = ["MALL_NAME","CONT_NO","COMPANY_CODE","COMPANY_NAME","BRAND_NAME","STORE_NO","READING_DATE","CHARGE_YM","FIN_YM","WATERRATE_CODE","DEVICE_NAME","HYDROELETRIC_TYPE","PRIOR_COUNT","READNUM","USE_COUNT","CONTRACT_PRICE","MONEY_RATE","RECE_NO","RATE","CREATE_NAME","CREATED_DATE","PROPERTY_AUDITE_NAME","PROPERTY_AUDIT_DATE","IS_PROPERTY_AUDIT","FINANCIAL_AUDITE_NAME","FINANCIAL_AUDIT_DATE","IS_FINANCIAL_AUDIT"];
        let Arr = ["MONEY_RATE"];
        // 判断元素类型
        // 获取数据
        // 传入jq元素
        ...
        pageView.reportData = function (param) {
            ...
            reportObj.chargeYM = new Date().getFullYear() + month;

            // reportObj.chargeYM = (new Date()).Format("yyyyMM");
            // reportObj.finYM=new Date().getFullYear()+'-'+new Date().getMonth();
        ...
        pageView.ajax = function (param) {
            ...
            $.ajax({
                url: '/report_web/fixed_contract_receivable_report/getReportData.htm',
                type: 'POST',
                ...
                success: function (res) {
                    ...
                    if (reportData.length < 1) {
                        ...
                        $('#bb').empty();
                        let newTitles = res.data.newTitles;
                        let titles = "";
                        for (let i = 0; i < newTitles.length; i++) {
                            ...
                            t = t.replace('（', "<br/>（");
                            if (-1 != t.indexOf('（')) {
                                style = 'style="line-height:' + 15 + 'px;font-size:' + 14 +'px;"';
                            }
                            titles += '<th class="text-center" ' + style + ' title="' + newTitles[i] + '">' + t + '</th>';
                        }
                        $('#bb').html(titles);
        ...
        pageView.init = function (param) {
            ...
            //    财务月
            $('input[name=chargeYM]').attr('placeholder',result)
            //    业务月
            // $('input[name=finYM]').attr('placeholder',result)
    ...
    $(document).ready(function (param) {
        ...
        $('#exportAll').click(function () {
            ...
            formPost("/report_web/fixed_contract_receivable_report/exportAll.htm", pageView.reportData());
        });
        ...
        $('#export').click(function () {
            ...
            formPost("/report_web/fixed_contract_receivable_report/export.htm", pageView.reportData());
        });
