$(function (param) {
    // 日期
    let pageView = (function ($) {
        let pageView = {};
        let urlArr=[
            '/report_web/margin_details_report/report.htm',
            '/report_web/receipt_details_report/report.htm',

            "/report_web/booking_details_report/report.htm",
            "/report_web/cont_sales_and_survey_report/report.htm",
            "/report_web/deposit_balance_report/report.htm",
            "/report_web/invoice_details_report/report.htm",
            "/report_web/margin_balance_report/report.htm",
            "/report_web/new_system_verification_report/report.htm",
            "/report_web/property_energy_equipment_report/report.htm",
            "/report_web/property_hydropower_input_report/report.htm",
            "/report_web/real_time_arrears_report/report.htm",
            "/report_web/receivable_received_report/report.htm",
            "/report_web/tenants_owe_report/report.htm",
            "/report_web/cont_sales_and_survey_report/report.htm",

            '/report_web/advanceCalculateLogic/report.htm',
			'/report_web/fin_invoice/report.htm',
            '/report_web/keep_logic/report.htm',
            '/report_web/contInfoReport/report.htm'
        ];
        let sassCokie=JSON.parse(sessionStorage.getItem('checkAll')||'{}');
        // 插件部分
        // 初始时间
        pageView.initialTime = function (date) {
            let Year = date.getFullYear();
            let Month = (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1);
            let Day = (date.getDate() < 10 ? '0' + date.getDate() : date.getDate());
            let data = Year + '-' + Month + '-' + Day;
            return data;
        };
        // 清除文字两边的空格
        pageView.trim = function (str) {
            return str.replace(/(^\s*)|(\s*$)/g, "");
        }
        // 2级菜单
        pageView.towmenu = function (param) {

        }
        /*//隐藏折叠面板
        // data-toggle="collapse" data-target="#report_senior_condition" aria-expanded="true"
        pageView.toggle=function(){
            $('.closeX').on('click',function () {
                $('.lookup').attr({
                    'data-toggle':'collapse',
                    'data-target':'#report_senior_condition',
                    'aria-expanded':true
                })
            })
            $('.lookup').on('click',function () {
                    $(this).attr({
                        'data-toggle':'',
                        'data-target':'',
                        'aria-expanded':''
                    })
            })
        }*/
        // 页面操作部分
        // 设置时间

        pageView.dateMonth = function (url) {
            let datepickerMonth = $(".dateMonth").find("input").datetimepicker({
                format: "yyyy-mm",
                todayBtn: "linked",
                clearBtn:"linked",
                startView: 3,
                minView: 3,
                autoclose: true,
                language: "zh-CN",
            }).on('changeDate', function (e) {

            });
        };
        pageView.dateDay = function () {
            let datepickerDay = $(".dateDay").find("input").datetimepicker({
                format: "yyyy-mm-dd",
                todayBtn: "linked",
                clearBtn:"linked",
                startView: 2,
                minView: 2,
                autoclose: true,
                language: "zh-CN",
            }).on('changeDate', function (e) {
            });
        };

        pageView.dateMonthUnCk = function () {
            let date = new Date();
            let mm = date.getMonth();
            if (0 == mm) {
                date.setFullYear(date.getFullYear() - 1);
                mm = 12;
            }
            let month = ~~(mm) < 10 ? '0' + mm : mm;
            let result = date.getFullYear() + '-' + month;

            let dateMonthUnCk = $(".dateMonthUnCk").find("input").datetimepicker({
                format: "yyyy-mm",
                startView: 3,
                minView: 3,
                autoclose: true,
                initialDate : new Date(result),
                language: "zh-CN",
            }).on('changeDate', function (e) {
            });
        };

        pageView.dateMonthUnCkNow = function () {
            let date = new Date();
            let mm = date.getMonth() + 1;
            let month = ~~(mm) < 10 ? '0' + mm : mm;
            let result = date.getFullYear() + '-' + month;

            let dateMonthUnCkNow = $(".dateMonthUnCkNow").find("input").datetimepicker({
                format: "yyyy-mm",
                startView: 3,
                minView: 3,
                autoclose: true,
                initialDate : new Date(result),
                language: "zh-CN",
            }).on('changeDate', function (e) {
            });
        };

        pageView.dateMonthUnCkClean = function () {
            let date = new Date();
            let mm = date.getMonth();
            if (0 == mm) {
                date.setFullYear(date.getFullYear() - 1);
                mm = 12;
            }
            let month = ~~(mm) < 10 ? '0' + mm : mm;
            let result = date.getFullYear() + '-' + month;

            let dateMonthUnCk = $(".dateMonthUnCkClean").find("input").datetimepicker({
                format: "yyyy-mm",
                startView: 3,
                minView: 3,
                clearBtn:"linked",
                autoclose: true,
                initialDate : new Date(result),
                language: "zh-CN",
            }).on('changeDate', function (e) {
            });
        };

        //储存当前选着的数据
        pageView.sessIon=function (bool,id) {
            sassCokie[id]=bool;
            sessionStorage.setItem('checkAll',JSON.stringify(sassCokie));

        };
        //单选全选
        pageView.checAll=function () {
            $('.all').on('click',function () {
                if($(this).hasClass('checked')){
                    $('.all').removeClass('checked');
                    $("em[class='zl-checkbox check checked']").parents('tr').attr('data-chk',false)
                    $("em[class='zl-checkbox check checked']").removeClass('checked');
                    $('.checkOne').find('tr').each((index,item)=>{
                        pageView.sessIon('false',$(item).attr('data-data'));
                    })
                }else{
                    $('.all').addClass('checked');
                    $("em[class='zl-checkbox check']").parents('tr').attr('data-chk',true)
                    $("em[class='zl-checkbox check']").addClass('checked');
                    $('.checkOne').find('tr').each((index,item)=>{
                        pageView.sessIon('true',$(item).attr('data-data'));
                    })
                }
            })
//
            $('.checkOne').on('click',function (event) {
                let target=$(event.target);
                if(target.is('em')){
                    let num=0;
                    let emk= $('.checkOne').find('em')
                    if(target.hasClass('checked')){
                        target.parents('tr').attr('data-chk',false);
                        target.removeClass('checked');
                    }else{
                        target.parents('tr').attr('data-chk',true);
                        target.addClass('checked');
                    }
                    emk.each((index,elm)=>{
                        if($(elm).hasClass('checked')==false){
                            $('.all').removeClass('checked');
                            return false;
                        }
                        num++;
                    })
                    if(num==$('.checkOne').find('tr').length){
                        $('.all').addClass('checked');
                    }
                    let bool=target.parents('tr').attr('data-chk');
                    let id=target.parents('tr').attr('data-data');
                    pageView.sessIon(bool,id)
                }
            })



        }
        // 时间
        pageView.iniTime = function () {
            let date = new Date();
            let time = this.initialTime(date);
            $('.start-time').find('input').val(this.initialTime(new Date(date.getTime() - 2592000000)));
            $('.end-time').find('input').val(time);
        }
        // 清空
        // 给你需要清空的选项添加data-Eliminate并填写好你要替换的值如（data-Eliminate=''）
        pageView.eliminate = function (param) {
            $('.report-eliminate').on('click', function (param) {
                let att = $('#report_senior_condition').find('[data-Eliminate]');
                att.each((index, elm) => {
                    let item = $(elm);
                    let elim = item.attr('data-Eliminate');
                    if (item[0].tagName == 'INPUT') {
                        item.val(elim);
                        $('#layoutButton2').next().find('li').eq(0).siblings().remove();
                        $('#layoutButton3').next().find('li').eq(0).siblings().remove();

                    }
                    if (item[0].tagName == 'BUTTON') {
                        item.text(elim);
                    }
                })
            })
        }
        // 下拉选项
        pageView.dropDown = function () {
            // 基础查询
            $('.zl-date-select').on('click', function (event) {
                let target = $(event.target);
                // 选取下拉 (小bug第二次点击时会执行两次)
                if (target.is('button')) {
                    let buTex = target.text();
                    target.next('ul').find('li').each((index, elm) => {
                        let item = $(elm);
                        let elmTex = item.find('a');
                        elmTex.on('click', function () {
                            target.prev().val(elmTex.attr('data-value'))
                            target.html($(this).text());

                        })
                    })
                }
            });
            // 高级查询
            $('.senior').on('click', function (event) {
                let target = $(event.target);
                // 选取下拉 (小bug第二次点击时会执行两次)
                if (target.is('button')) {
                    let buTex = target.text();
                    target.next('ul').find('li').each((index, elm) => {
                        let item = $(elm);
                        let elmTex = item.find('a');
                        elmTex.on('click', function () {
                            target.prev().val(elmTex.attr('data-value'))
                            target.html($(this).text());
                        })
                    })
                }
            })
            //选着显示的页数
            $('#largeNumBtn').on('click',function () {
                let _this=$(this);
                let buTex = _this.text();
                _this.next('ul').find('li').each((index, elm) => {
                    let item = $(elm);
                    let elmTex = item.find('a');
                    elmTex.on('click', function () {
                        _this.prev().val(elmTex.attr('data-value'))
                        _this.html(elmTex.text());
                    })
                })

            })
        }

        ///swiper
        pageView.swiper=function () {
            var ys_main_swiper = new Swiper('#zl-floor-main-table', {
                scrollbar: '.swiper-scrollbar-a',
                direction: 'horizontal',
                slidesPerView: 'auto',
                mousewheelControl: true,
                freeMode: true,
                freeModeMomentum:false,
                scrollbarHide: false,
                scrollbarDraggable: true,
                preventClicksPropagation: false,
            });
            ys_main_swiper.on('setTranslate',function (translate) {
                let le=ys_main_swiper.getWrapperTranslate()>=0?0:ys_main_swiper.getWrapperTranslate();
                if(le>=0) {
                    $('.swiper-wrapper').css('transform', "translate3d(" + le + "px, 0px, 0px)")
                }
            })

        }
        // 查询
        $('.lookup').on('click', function () {
            $('#report_senior_condition').collapse('hide');
        })
        pageView.init = function () {
            // ys_main_swiper.on('setTranslate',function (translate) {
            //     let le=ys_main_swiper.getWrapperTranslate()>0?0:ys_main_swiper.getWrapperTranslate();
            //     $('.transi').css('transform',"translate3d("+parseInt(Math.abs(le))+"px, 0px, 0px)")
            //
            // })
            let url=location.pathname;
            pageView.checAll();
            pageView.iniTime();
            pageView.dateDay();
            pageView.dateMonth(url);
            pageView.dateMonthUnCk();
            pageView.dateMonthUnCkClean();
            pageView.dateMonthUnCkNow();
            pageView.dropDown();
            pageView.eliminate();
            if(urlArr.indexOf(url)!=-1){

            }else{
                pageView.swiper();
            }
            // urlArr.map((index,item)=>{
            //     if(url==item){
            //         return;
            //     }else{
            //
            //     }
            // })

            // $(".dateMonthUnCk").datetimepicker("setDate", new Date("2017-03-19"));
        };
        return pageView;

    })(jQuery);

    $(document).ready(function () {
        pageView.init();
    });

})