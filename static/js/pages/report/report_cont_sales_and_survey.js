$(function (param) {
    let pageView = (function (param) {
        ...
        let itemsPerPage=10;
        let pageView = {};
        let heaArr=["brandName", "contNo", "categoryCode", "itemName", "blockName", "floorName", "storeNos", "companyCode", "contType", "centReceType", "confirmAmount", "excelAmount", "opsAmount", "netAmount", "saleAmountBefore", "saleAmountAfter", "updatedDate"];
        ...
        pageView.init = function (param) {
            Api.locaShow();
            pageView.compane();
            Api.init('/report_web/cont_sales_and_survey_report/getReportData.htm',pageView.reportData(),'reportData','newTitles');
        ...
        //..
        pageView.compane=function(){
            let begin = 0;
            let end = 0;
            $('#beginSaleYm').on('change',function () {
                begin = new Date($('#beginSaleYm').val()).getTime();
                end = new Date($('#endSaleYm').val()).getTime();
                if (begin > end && 0 != end) {
                    $(this).val('');
                    return alert('开始日期不能大于结束日期')
                }
            })
            $('#endSaleYm').on('change',function () {
                begin = new Date($('#beginSaleYm').val()).getTime();
                end = new Date($('#endSaleYm').val()).getTime();
                if (end < begin && 0 != begin) {
                    $(this).val('');
    ...
    $(document).ready(function (param) {
        ...
        $('#exportAll').click(function () {
            ...
            formPost("/report_web/cont_sales_and_survey_report/exportAll.htm", pageView.reportData());
        });
        ...
        $('#export').click(function () {
            ...
            formPost("/report_web/cont_sales_and_survey_report/export.htm", pageView.reportData());
        });
