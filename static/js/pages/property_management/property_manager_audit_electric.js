(function ($) {
    ...
    $("#searchType").on("click", function (e) {
        $("#searchReForm").attr("action", managementWeb_Path + "propertyManagerAudit/water/index.htm").submit();
    });
    /**
     * 下拉初始化
     */
    ...
    $("#searchBsFloorForm").submit(function () {
        ...
        self.attr("action", managementWeb_Path + "propertyManagerAudit/electric/index.htm");
    });
    /**
     * 审核
     */
    $("#examineButton").on("click",function (e) {
        ...
        $(".zl-fake-checkbox.checked:not(.all)").each(function() {
            ...
            if($("#isPropertyAudit_"+id).val()==1){
                flag = false;
                alert("选中有已审核的记录，请重新选择后再操作");return;
            }
            ids.push(id);
        });
        if(flag){
            confirm("确认要审核吗？","","",function (type) {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    dataType: "json",
                    url: managementWeb_Path + "propertyManagerAudit/electric/audit.htm",
     ...
     * 取消审核
     */
    $("#cancelExamineButton").on("click",function (e) {
        if ($(".zl-fake-checkbox.checked").length == 0) {
            alert("请选择后再操作");return;
        }
        var ids = [];
        ...
        $(".zl-fake-checkbox.checked:not(.all)").each(function() {
            ...
            if($("#isPropertyAudit_"+id).val()!=1){
                flag = false;
                alert("选中有未审核的记录，请重新选择后再操作");return;
            }
            if($("#isFinancialAudit_"+id).val()==1){
                flag = false;
                alert("选中记录存在财务已审核，不允许取消操作");return;
            }
            ids.push(id);
        });
        if(flag){
            confirm("确认要取消审核吗？","","",function (type) {
                if(type=="dismiss"){
                    return;
                }
                loadingshow();
                $.ajax({
                    ...
                    type: "POST",
                    dataType: "json",
                    url: managementWeb_Path + "propertyManagerAudit/electric/cancel.htm",
                    data: {ids : ids.toString()},
                    //async: false,
                    error: function (request) {
                        loadingHide();
