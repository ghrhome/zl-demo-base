...
(function ($) {
    ...
    //     $("#searchReForm").attr("action", managementWeb_Path + "hydroelectric/water/index.htm").submit();
    // });
    ...
    $("#daoru").click(function () {
        $("<input type='file' name='file' style='display: none'/>").appendTo("body").change(function () {
            ...
            $.ajax({
                url: managementWeb_Path+'hydroelectric/electric/upload-excel.htm',
                type: 'POST',
                dataType: "json",
                cache: false,
                data: formData,
    ...
    $("#js-view").on("click",function(e){
        $.ajax({
            cache: true,
            type: "POST",
            url: managementWeb_Path + "hydroelectric/electric/veliList.htm",
            async: false,
    ...
    $("#addBsFloorBtn").on("click", function () {
        ...
        $.ajax({
            cache: true,
            type: "POST",
            url: managementWeb_Path + "hydroelectric/electric/velicateName.htm",
        ...
        $.ajax({
            cache: true,
            type: "POST",
            url: managementWeb_Path + "hydroelectric/electric/add.htm",
            data: $('#addBsFloorForm').serialize(),
            async: false,
            error: function (request) {
                alert("系统异常");
    ...
    $("[name=updateBsFloorBtn]").on("click", function () {
        ...
        confirm("确认要提交吗？","","",function (type) {
            ...
            $.ajax({
                cache: true,
                type: "POST",
                url: managementWeb_Path + "hydroelectric/electric/update.htm",
                data: $('#updateBsFloorForm_' + mngHydroelectricId).serialize(),
                async: false,
                dataType:"json",
                error: function (request) {
    ...
    $("[name=deleteBsFloorBtn]").on("click", function () {
        ...
        confirm("确认要删除吗？","","",function (type) {
            ...
            $.ajax({
                cache: true,
                type: "POST",
                url: managementWeb_Path + "hydroelectric/electric/delete.htm?mngHydroelectricId=" + mngHydroelectricId,
                async: false,
                error: function (request) {
                    alert("系统异常");
                },
...
//00 var re = /^(\-?)[0-9]*[0-9][0-9]*$/;
    ...
    $("#searchBsFloorForm").submit(function () {
        ...
        self.attr("action", managementWeb_Path + "hydroelectric/electric/index.htm");
    });
...
function valitateForm(obj) {
    ...
    var rate  =$.trim($(obj).find("input[name=rate]").val());
    ...
    if (lastCount == "" || lastCount == null) {
        alert("最后读数不能为空！");
        return false;
    }/*else if(!isfoltNum(lastCount)){
     alert("新表底数格式错误，允许输入整数或小数！");
     return false;
     }*/
...
function shopTypeSelect(_this){
    ...
    $.ajax({
        cache: true,
        type: "POST",
        url: managementWeb_Path + "hydroelectric/electric/shopList.htm?mallId=" + mallId+"&&storeType="+storeType,
        async: false,
        error: function (request) {
            alert("系统异常");
        },
...
function ibrandSelect(_this){
    ...
    $.ajax({
        cache: true,
        type: "POST",
        url: managementWeb_Path + "hydroelectric/electric/ibrandList.htm?shopNum=" + shopNum+"&&storeType="+storeType,
        async: false,
        error: function (request) {
            alert("系统异常");
        },
