var expenseVerificationAddView = (function($){
    var expenseVerificationAddView = {};
    ...
    var _renterData=[];
    expenseVerificationAddView.search=function () {
        $("#js-dropdown-companys").ysSearchSelect({
            ...
            callback:function(value,ui){
                ...
                //expenseVerificationAddView.searchUpdate();
            },
        });
    }
    ...
    function _searchCb(companyId,companyName){
        $.getJSON(financeWeb_Path + 'finance/receivepaper/contlist.htm', {companyId: companyId}, function (res) {
            ...
            for (var i = 0; i < res.length; i++) {
                $('#js-dropdown-conts').find(' .dropdown-menu').append("<li><a key='" + res[i].contNo + "' href='javascript:void(0)' onclick='searchIsCollection(this);'>" + res[i].contNo + "</a></li>");

            }
        });
        var mallId = $('#mallId').val();
        var receiveAccount =  $('#receiveAccount').val();
        var receiveAccountName =  $('#receiveAccountName').val();
        $.getJSON(financeWeb_Path + 'finance/expenseVerification/recelist.htm', {saveCompanyId: companyId,saveCompanyName: companyName,mallId: mallId,receiveAccount: receiveAccount}, function (res) {
            if (res.length === 0) {
            ...
            for (var i = 0; i < receList.length; i++) {
                ...
                var receRightTr = "<tr class='trData'>"
                    ...
                    +"<input type='number' name='finExpenseVerificationReceEntityList[" + i + "].newDeAmount' min='0' onkeyup='clearNoNum(this)' class='text-right' value='" + newDeAmount + "'>"
                    +"</td>"
    ...
    expenseVerificationAddView.searchInit=function(){
    ...
    expenseVerificationAddView.searchUpdate=function(){
        console.log(_renterData)
    ...
    page.on("click",'.dropdown-menu li',function (e) {
        ...
        if ($(this).parents('#js-dropdown-companys')[0]) {
            $.getJSON(financeWeb_Path + 'finance/receivepaper/contlist.htm', {companyId: mallValue}, function (res) {
                $('#js-dropdown-conts').children('button').html('');
                $('#js-dropdown-conts').children('input').val('');
                if (res.length === 0) {
                    alert('此租户下没合同,请先添加合同或者选择别的租户');
                ...
                $('#js-dropdown-conts').find(' .dropdown-menu').children('li').remove();
                for (var i = 0; i < res.length; i++) {
                    $('#js-dropdown-conts').find(' .dropdown-menu').append("<li><a key='" + res[i].contId + "' href='javascript:void(0)'>" + res[i].contNo + "</a></li>");
                }
            });
        }
    });
    ...
    $("#js-addnew-save").on("click", function (e) {
        ...
        //var isChecked = true;
        if (isChecked) {
            expenseVerificationAddView.loadingShow();
            $.ajax({
                type: "POST",
                dataType: "json",
                ...
                success: function (resultData) {
                    expenseVerificationAddView.loadingHide();
                    if (resultData.success == true) {
                        alert("保存成功","","",function(){

                            var _href="#/fin_receipt_verification";
    ...
    function bindPageEvents() {
        ...
        $(".reOffsetAmount").on("input", function () {
            ...
            $(".realOffsetAmount").each(function () {
                ...
                if(('预收租金' == offsetType || '预收水费' == offsetType || '预收电费' == offsetType || '预收服务费' == offsetType)) {
                    var canOffsetAmount = Number($(this).parents("tr").find(".canOffsetAmount input").val());
                    console.log('-----canOffsetAmount' + canOffsetAmount);
                    console.log('-----reOffsetAmountTotal' + reOffsetAmountTotal);
    ...
    $(".zl-glyphicon-blue").on("click", function (){
        ...
        expenseVerificationAddView.init();
    })
    ...
    expenseVerificationAddView.init = function(){
        $("#preloader").fadeOut("fast");
        //expenseVerificationAddView.searchInit();
        expenseVerificationAddView.search();
        bindPageEvents();
    };

    expenseVerificationAddView.loadingShow=function(){
        $(".zl-loading").fadeIn();
    };

    expenseVerificationAddView.loadingHide=function(){
        $(".zl-loading").fadeOut();
    }

    return expenseVerificationAddView;
})(jQuery);
                            ...
                            for (var key in _selectedAccounts) {
                                ...
                                if (tr.length == 0) {
                                    ...
                                    var receLeftTr = "<tr class='trData' data-fincontid='" + _selectedAccounts[key].id + "'>"
                                        ...
                                        +"</td>"
                                        + "<td class='text-right zl-edit'>"
                                        /*+ "<select name='finExpenseVerificationReceEntityList[" + inds + "].isCollection' id='isCollection' class='select btn-group zl-dropdown-inline'>"
                                    ...
                                    var receRightTr = "<tr class='trData' data-fincontid='" + _selectedAccounts[key].id + "'>"
                                        ...
                                        +"</td>"
                                        +"<td class='text-right zl-edit newDeAmount'>"
                                        +"<input type='number' name='finExpenseVerificationReceEntityList[" + inds + "].newDeAmount' min='0' onkeyup='clearNoNum(this)' class='text-right' value='" + newDeAmount + "' >"
                                        +"</td>"
                                        +"<td class='text-right zl-edit reOffsetAmount'>"
...
function checkCompanyName(param){
    if(param == '未明租户'){
        $(".changeCompany").show();
    }
}
