...
(function (factory) {
        ...
        function _createDom(data,prefix){
            ...
            $.each(data,function(i,e){
                ...
                if(typeof e.children!=="undefined" && e.children!==null && (e.children.length>0)){
                    ...
                    if(mergedOptions.tableRowTemplate){
                        var _data=e.data;
                        //console.log("======data=======")
                        //console.log(_data);
                        var _row=mergedOptions.tableRowTemplate(_data);

                        //console.log(_row);
                    }

                    var li="<li data-index='"+prefix+i+"' data-value='"+e.nodeValue+"' data-name='"+e.nodeName+"' data-id='"+e[mergedOptions.nodeNameSpace]+"' class='zl-tree-item parent"+(e.expanded ? ' open' : '')+"'><div class='zl-tree-table-row'>"+ "<span>"+e.nodeName+"</span>"+_row+"</div>"+
                        child +
                        "</li>";
                    ...
                    var li=( "<li data-index='"+prefix+i+"' data-value='"+e.nodeValue+"' data-name='"+e.nodeName+"' data-id='"+e[mergedOptions.nodeNameSpace]+"' class='zl-tree-item"+(e.expanded ? ' open' : '')+(_active ? ' active' : '')+"'><div class='zl-tree-table-row'>"+ "<span>"+e.nodeName+"</span>"+_row+"</div>"+ "</li>");
    ...
    var methods={
         init:function(options) {
             ...
             mergedOptions = $.extend(
                 true, {}, $.fn.ysListTreeTable.defaults, options,
                 {
                     //内部附加的属性，比如获取到的子元素
                     current: 0,
                 }
    ...
    $.fn.ysListTreeTable=function(method){
